...
### ✅ CHECKPOINT 14 — Segment duplication (data correctness)

**Goal:** One row per segment. Found while investigating the Splits tab; not originally reported.

`SegmentEntity` used `@PrimaryKey(autoGenerate = true) val id: Long = 0`, and `SegmentDto.toEntity`
rehydrated pulled rows with `id = 0`. Room reads 0 on an autoGenerate key as "assign a fresh one",
so `OnConflictStrategy.REPLACE` had nothing to match and **every Firestore `pullAll` re-inserted a
trip's entire segment set** — once per sign-in and per cold start. `pushSegment` had the mirror
problem: it keyed the Firestore document on the device-local autogenerated number.

Measured on the real 173 km / 1.5 h drive before the fix: **10,236 rows for ~853 real segments**
(each duplicated exactly 12×), segment durations summing to **17.71 h** and distances to **1,489.8 km**.
That is what made the Splits tab repeat identical rows and the header read `▴16:18:02`.

- [x] `SegmentEntity` — `@PrimaryKey val id: String` plus `idFor(tripId, segmentIndex)` =
      `"<tripId>#<segmentIndex>"`. Used for the Room key *and* the Firestore document id.
- [x] `SegmentDto.toEntity` **re-derives** the id rather than trusting the remote document id, so
      documents written by the old build collapse onto the right row on pull.
- [x] `FirestoreDataSource.pushSegment` keys on `segment.id` — pushes are now idempotent per device.
- [x] `TripRepositoryImpl.finishTripSegments` deletes the trip's segments before inserting, so
      re-processing a trip into *fewer* segments can't leave surplus rows behind.
- [x] `MIGRATION_2_3` rebuilds the table with the TEXT key and collapses existing duplicates via
      `GROUP BY tripId, segmentIndex`. Not destructive — route points are local-only and must survive.
- [x] 4 unit tests (`SegmentDtoTest`) pinning id derivation and pull idempotency. **33 tests green.**
- [x] **Acceptance test:** ✅ Verified on the emulator against the real duplicated database.
      Migration: **14,644 → 2,010 rows**, with `count(*) == count(distinct tripId, segmentIndex)` —
      zero duplicates. The 173 km drive: **10,236 → 853** segments, durations now summing to
      **1.48 h** against a 1.5 h trip. Ran **three cold starts** (each runs `pullAll`): the count
      held at 2,010 every time. Trip Detail header now reads **`▴4:04`** instead of `▴16:18:02`, and
      the Splits rows show distinct times (0:02.3 / 0:05.7 / 0:01.3) instead of seven identical ones.

> **Leftover:** the remote `/users/{uid}/segments/` collection still holds the old duplicate
> documents under their former numeric ids. Pulls now collapse them correctly so there is no
> user-visible bug, but they make every pull heavier. Worth clearing the collection once from the
> Firebase console; the next push rewrites a clean set under the derived ids.

> **Not fixed here (see `DISCUSSION.md`):** 853 segments for 173 km is still far too granular, and
> the segment distances sum to 124.1 km against a 173.1 km trip (~28% under) because of RDP thinning
> plus dropped one-point groups. Both belong to the segmentation rework.
